...
final class HistoricalAvisoParser
{
    ...
    private static function extractPedimentos(string $upper): array
    {
        ...
        $seen = [];
        $patterns = [
            '/PEDIMENTO(?:\s+DE\s+IMPORTACION(?:\s+TEMPORAL)?)?\s+CLAVE\s+([A-Z0-9]{1,4})\s*(?:N[ÚU]MERO\s*:\s*)?([0-9][0-9 ]{10,24})/u',
            '/CLAVE\s+([A-Z0-9]{1,4})\s*[:\-]?\s*([0-9][0-9 ]{10,24})/u',
        ];
        foreach ($patterns as $pattern) {
            ...
            foreach ($matches as $match) {
                ...
                $number = self::normalizePedimentoNumber($match[2]);
                $identity = $key . ':' . $digits;
                if (isset($seen[$identity])) {
                    continue;
                }
                $seen[$identity] = true;
                $results[] = ['key' => $key, 'number' => $number];
            }
        }
        return $results;
    }
    ...
    private static function extractItems(string $raw, array $pedimentos): array
    {
        ...
        foreach ($matches as $index => $match) {
            ...
            $key = null;
            $number = null;
            if (preg_match('/PEDIMENTO(?:\s+DE\s+IMPORTACION(?:\s+TEMPORAL)?)?\s+CLAVE\s+([A-Z0-9]{1,4})\s*:?\s*([0-9][0-9 ]{10,24})/iu', self::upper($tail), $pm)) {
                $key = strtoupper(trim($pm[1]));
                $digits = preg_replace('/\D+/', '', $pm[2]) ?? '';
                if ($digits !== '') {
                    $number = self::normalizePedimentoNumber($pm[2]);
                }
            } elseif (count($pedimentos) === 1) {
                $key = $pedimentos[0]['key'];
                $number = $pedimentos[0]['number'];
            }
            $partida = null;
        ...
        foreach ($items as $item) {
            ...
            $pedimentoKey = (string) preg_replace('/\D+/', '', (string) ($item['pedimento'] ?? ''));
