...
const PRECACHE_URLS = [
  ...
  './assets/js/aviso-expediente-merchandise.js',
  './assets/js/aviso-import.js',
  ...
  if (isDynamicRequest(request, requestUrl)) {
    event.respondWith(
      fetch(request, { cache: 'no-store' }).catch(() =>
        new Response('Sin conexión. Vuelve a intentarlo cuando recuperes la red.', {
          status: 503,
          headers: {
...
self.addEventListener('message', (event) => {
  ...
  if (!data || data.type !== PROCESS_QUEUE_MESSAGE) {
    return;
  }
